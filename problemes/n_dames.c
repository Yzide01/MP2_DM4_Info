...
char *contrainte_une_diagonale2(int i,int n ){
    char **l=malloc((n-abs(i))*sizeof(char*));

    
    if (i<0){
        i= -i ;//diagonale partant de la colonne tout à droite
        int colonne=n-1; 
        for(int j=i;j<n;j++){
            l[j-i]=variable(j,colonne);
            colonne--;
        }
...
char *contrainte_toute_diagonale2(int n ){
    char** l = malloc(((2*n)-1)*sizeof(char*));
        
    l[0]=contrainte_une_diagonale2(0,n);

        
    for (int i = -1; i > (-n); i--){
        //printf("%d\n",i);
        l[-i] = contrainte_une_diagonale2(i,n);
    }
        
    for(int i=1;i<n ;i++){
        //printf("%d\n",i);
        l[i+n-1]=contrainte_une_diagonale2(i,n);
        //printf("%s\n\n",l[i+n-1]);
    }
    char* res = separation_et (l,(2*n)-1);
    ...
    char *contrainte4=contrainte_toute_diagonale2(n);
    ...
    char *formule_resolution_pb_n_dames=separation_et(l,4);;
    free(l[0]);
    free(l[1]);
    free(l[2]);
    free(l[3]);
